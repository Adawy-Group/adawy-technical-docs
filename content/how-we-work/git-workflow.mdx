...
1. **`main` is always deployable.** It is protected: no direct pushes, changes
   land via reviewed pull requests only.
...
running pages, not screenshots. Automated checks (build, types, lint) must
pass before review; a red check is the author's to fix, not the reviewer's to
catch.
