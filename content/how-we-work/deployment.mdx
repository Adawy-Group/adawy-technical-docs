...
<Callout type="info">
  **That table is the pipeline where it fires on its own, which is not
  everywhere** — [`jumeira`](/repositories/jumeira) has no automatic trigger at
  all ([why, and what to do instead](#client-projects-deploy-the-same-way)).
  Read a repo's own workflow before assuming a merge deployed anything.
</Callout>
...
Three repos deploy differently, and all three are worth knowing before you push:
...
- **`jumeira`'s CI is manual.** The workflow gates and deploys exactly as
  described above, but only when someone dispatches it, so **merging to `main`
  ships nothing** and a green history proves nothing about a commit nobody ran
  it on. Run all five gates locally before pushing, and either dispatch the
  workflow or deploy by hand:
  ...
  The Vercel Git integration is not an option there: the project is on a plan
  that cannot connect a private organisation-owned repo, which is the same
  constraint that put the platform on the CLI.
