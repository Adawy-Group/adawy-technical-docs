...
flowchart LR
    ...
    R1["/api/contact"] --> S
    R2["/api/quote"] --> S
    S["lib/enquiry.ts\nthe only place a submission leaves"] --> P["Resend · a log line · whatever is next"]
```
...
```mermaid
flowchart TD
    A["A fact or a behaviour appears twice"] --> B{"Does the second copy\nrender identically when wrong?"}
    B -- yes --> C["Write the test\npattern 6"]
    B -- no --> D{"Do the two copies have to\nagree forever?"}
    D -- yes --> E["Give it a typed handle\npattern 3"]
    D -- no --> F{"Is it a boundary —\nsomething leaving the app?"}
    F -- yes --> G["Give it a seam\npattern 1"]
    F -- no --> H["Leave it duplicated.\nExtract on the third use"]
```
