...
Before anything else: the app has three layers, and **the arrow that is not
allowed is the point.** The dotted edge below is a `no-restricted-imports`
error — the eslint config *generates* those blocks from the layering, so the
rule and the structure cannot drift apart, and CI fails on a breach.
...
```mermaid
flowchart LR
    A["lib/&lt;section&gt;/<br/>pure math — beats and easing<br/>no React, no three.js"] --> B["progress channel<br/>module singleton<br/>clamped to 0..1"]
