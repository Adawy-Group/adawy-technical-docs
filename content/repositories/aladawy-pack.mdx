...
`aladawypack.com` itself still serves the client's old WordPress site, and its
DNS and email live on the group's Verpex host. Moving it is
[Moving a Domain to Vercel](/guides/move-a-domain-to-vercel), with the email
moved first, since the client's staff use those mailboxes.
